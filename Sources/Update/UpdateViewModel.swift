...
class UpdateViewModel: ObservableObject {
    ...
    static func userFacingErrorMessage(for error: Swift.Error) -> String {
        ...
        if let networkError = networkError(from: nsError) {
            switch networkError.code {
            case NSURLErrorNotConnectedToInternet:
                return String(localized: "update.error.noInternet.message", defaultValue: "c11mux can’t reach the update server. Check your internet connection and try again.")
            case NSURLErrorTimedOut:
            ...
            case NSURLErrorCannotConnectToHost:
                return String(localized: "update.error.serverUnreachable.message", defaultValue: "c11mux couldn’t connect to the update server. Check your connection or try again later.")
            case NSURLErrorNetworkConnectionLost:
        ...
        if nsError.domain == SUSparkleErrorDomain {
            switch nsError.code {
            case 2001:
                return String(localized: "update.error.feedDownload.message", defaultValue: "c11mux couldn't download the update feed. Check your connection and try again.")
            case 1000, 1002:
            ...
            case 1003, 1005, 4005:
                return String(localized: "update.error.permissionError.message", defaultValue: "Move c11mux into Applications and relaunch to enable updates.")
            default:
                break
            }
        }
