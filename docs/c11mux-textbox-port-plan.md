# c11mux — TextBox Input Port Plan
...
Port the TextBox Input feature from the alumican/cmux-tb fork into c11mux. The feature adds a native macOS text-input box mounted below each terminal surface, giving users (and AI agents) a friendlier way to compose and submit multi-line text to the shell.
...
**Shape of the port**: Selective forward-port, not a merge. The fork is 189 commits behind upstream and has fork-specific release/branding artifacts we do not want. We copy the one new file verbatim and hand-apply the integration hooks to c11mux's current `main`.
...
For each file, `delta` is the fork's diff against upstream `main`. `c11mux risk` reflects how much the current c11mux file diverges in ways that could collide with the fork's edits.
...
| `cmuxTests/TextBoxInputTests.swift` | 360 | Copy; verify `@testable import` matches c11mux's target (fork uses `cmux_DEV` + `cmux`, matching c11mux's existing convention — so likely no change needed). Update `defaultEnabled` assertion to `false` to match our flipped default. Key assertion `"b"` already matches our chosen shortcut. |

**On the two-commit regression-test pattern.** CLAUDE.md's red-then-green split is for bug-fix PRs — it asserts the test genuinely catches the bug. These tests validate a newly-ported feature, not a c11mux bug fix. The pattern does not apply here. Call this out in the PR description to preempt reviewer confusion.

**Strip fork-author metadata.** Grep the file for `[cmux-tb]` and any `alumican`/`fork-release` references. These are maintenance markers tied to the upstream fork's release channel and must not ship in c11mux.
...
**c11mux risk**: none. Pure additions; no existing logic touched.
...
**c11mux risk**: low. c11mux has sidebar and theming work here but the insertion pattern (VStack + conditional view below terminal) slots cleanly underneath any existing content.

**Review point**: verify c11mux's `TerminalPanelView` still has the search-overlay layering constraint documented in CLAUDE.md — the overlay must continue to be portal-mounted from `GhosttySurfaceScrollView`, not from this panel. Mounting the TextBox below the terminal should not disturb this.
...
**Also add the title-sync hook.** Inside `updatePanelTitle` (c11mux `Workspace.swift` has a sibling of the fork's `updatePanelTitle`), when the panel is a `TerminalPanel` and the title actually changed, call `terminalPanel.updateTitle(trimmed)`. Fork adds this hook at its `Workspace.swift:7819–7823` with the comment *"Keep TerminalPanel.title in sync so TextBox key routing can detect running apps (Claude Code, Codex) by terminal process title."* Without this hook, the fork's title-regex agent detection silently never fires — shipping a broken `/`/`@`/`?` feature. Verified against fork source.

**c11mux risk**: low. c11mux has heavy churn in `Workspace.swift` around surface management and session restore, but the two additions (`toggleTextBoxMode` + title-sync line) are self-contained and do not touch existing lifecycle code.

**Review point**: confirm the focus-detection pattern (`firstResponder is InputTextView`) does not race with c11mux's own focus work — specifically any code that asynchronously restores focus after a workspace switch.
...
**Shortcut collision — `Cmd+Option+T` is already bound.** c11mux binds this chord at `Sources/AppDelegate.swift:9498` (hardcoded, not through `KeyboardShortcutSettings`) to `closeOtherTabsInFocusedPaneWithConfirmation()`. The binding is guarded by `cmuxTests/AppDelegateShortcutRoutingTests.swift`. The fork's own `KeyboardShortcutSettings.swift:265` contains an inline comment acknowledging this: *"Default: Cmd+Opt+T (upstream cmux PR uses Cmd+Opt+B to avoid conflict with close-other-tabs)."* We take the upstream-safe choice: **`Cmd+Option+B`**. Zero migration cost.
...
**c11mux risk**: none. Enum extension.
...
**c11mux risk**: very low. Pattern mirrors existing shortcut intercepts.
...
**c11mux risk**: low. Append-only addition to the settings panel.
...
**c11mux risk**: very low — all additive. But `GhosttyTerminalView.swift` contains `TerminalSurface.forceRefresh()`, which CLAUDE.md flags as typing-latency-sensitive. None of these additions touch that method.

**Verified**: c11mux's Ghostty submodule defines `scroll_to_row` in `src/input/Binding.zig` — the binding the TextBox uses for scroll preservation is supported.
...
**c11mux-specific wrinkle.** c11mux's drag system uses portal-based hit testing and `activeDragWebView`/`preparedDragWebView` state (see `Sources/ContentView.swift:607-685`). The fork's simpler recursive-walker approach will not drop in cleanly on top of that state machine. Phase 5 must read the *current* c11mux drag pipeline end-to-end before deciding whether to:
...
**c11mux risk**: moderate-to-high. c11mux's `ContentView.swift` has the most churn of any integration point, and this is the only part of the port that mutates (rather than appends to) an actively contested file.
...
**c11mux ships 18 locales** (verified): `en`, `ja`, `zh-Hans`, `zh-Hant`, `ko`, `ar`, `bs`, `da`, `de`, `es`, `fr`, `it`, `nb`, `pl`, `pt-BR`, `ru`, `th`, `tr`. CLAUDE.md is explicit: *"All user-facing strings must be localized... Keys go in `Resources/Localizable.xcstrings` with translations for all supported languages (currently English and Japanese)."*

**Plan**: port the fork's English AND Japanese values verbatim in Phase 7. For the other 16 locales, follow c11mux's existing pattern (leave the keys untranslated so they fall back to English until translation passes run). Shipping EN-only would violate policy; shipping EN+JA matches existing practice.
...
**c11mux risk**: none — additive; matches existing patterns.
...
**c11mux risk**: none. Standard Xcode file-add; do it through Xcode to avoid hand-editing the pbxproj.
...
| Artifact | Reason |
|---|---|
| `Resources/Info.plist` `SUFeedURL` change | Fork points Sparkle at its own appcast; would hijack c11mux's update channel. |
| `.github/workflows/release-tb.yml` | Fork-specific CI/release pipeline with alumican's signing identity. |
| `fork-release.md`, `upstream-sync.md` | Fork-maintenance docs, not applicable to c11mux. |
...
- `docs/assets/textbox-*.{gif,mp4,png}` — screencasts of the feature. Useful for c11mux's own changelog/README if we want to showcase the feature.
...
- `TerminalPanelView.swift`: mount `TextBoxInputContainer`, add `@AppStorage`, add `onChange` observers. Reconcile the signature drift (fork uses `paneId: PaneID`; c11mux does not — port the TextBox-relevant slice only).
...
- Add the fork's ~18 strings to `Resources/Localizable.xcstrings` with EN + JA values. Other 16 locales: leave untranslated and let them fall back to English (matches c11mux's existing pattern).
- Build check.
...
|---|---|---|
| Drag-routing regression in `ContentView.swift` | Medium-High | Lifecycle is 5+ entry points, not 1 (see §4.9). Read c11mux's current `draggingEntered`/`draggingUpdated`/`prepareForDragOperation`/`performDragOperation`/`concludeDragOperation` end-to-end before inserting. Manual test every existing drop target (web, browser, terminal, TextBox). Consider unifying the `activeDragWebView` state machine into an `activeDragTarget` enum. |
| Focus-handling race with c11mux's workspace restore | Medium | `toggleTextBoxMode` uses `DispatchQueue.main.async`; verify no re-entry during workspace switch. **Grep every `makeFirstResponder(surfaceView)` call site** and guard each with `if firstResponder is InputTextView { return }` — c11mux has focus-restore paths that did not exist in upstream (`scheduleAutomaticFirstResponderApply`, `reassertTerminalSurfaceFocus`). |
| Ghostty `scroll_to_row` unsupported | Resolved | Verified present in c11mux's submodule at `src/input/Binding.zig:427`. |
...
| `GhosttyApp.defaultForegroundColor` missing | **Active** | Symbol does not exist in c11mux. Add it in Phase 3 (see §4.8). Without this, Phase 4 fails to compile. |
...
| Coordination with tier-1 persistence plan | Medium | `docs/c11mux-tier1-persistence-plan.md` (in-flight) also touches per-panel state. `isTextBoxActive` and `textBoxContent` are candidates for future persistence. Sync with the tier-1 author before both land on `main`. |
...
| Sparkle update overwrites feature | N/A | We explicitly do not port the fork's `SUFeedURL` change; c11mux's appcast stays. |
...
15. **Persistence of TextBox content.** Fork is transient. c11mux has richer session restore + the in-flight tier-1 persistence plan. Save draft text per surface via `SurfaceMetadataStore`, or leave transient? **Recommend**: transient for v1, file follow-up.

16. **Translation breadth.** EN + JA ship (matches fork + policy). For the other 16 locales, leave untranslated (fall back to EN — matches current c11mux pattern) or batch-translate now?

17. **Showcase assets.** Port `docs/assets/textbox-*.{gif,mp4,png}` for c11mux's own docs/README?
...
- `TextBoxInputTests.swift` passes in c11mux's test target.
...
- Japanese translations land in the same PR (per policy); remaining locales fall back to English per c11mux convention.
