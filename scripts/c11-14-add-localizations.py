...
ENTRIES: list[tuple[str, str, str, str, str, str, str, str]] = [
    ...
    (
        ...
        "Applied to every new terminal surface (menu, keyboard, socket). Use “New Bash Terminal” to bypass for a single surface. Per-project overrides live in `.c11/agents.json`; per-workspace overrides live in workspace metadata (`default_agent_use_bash`, `default_agent_inline`).",
        "すべての新規ターミナル面 (メニュー、キーボード、ソケット) に適用されます。1 つの面だけ回避するには「新規 Bash ターミナル」を使用してください。プロジェクト単位の上書きは `.c11/agents.json` に、ワークスペース単位の上書きはワークスペースメタデータ (`default_agent_use_bash`、`default_agent_inline`) に置きます。",
        "Застосовується до кожного нового термінала (меню, клавіатура, сокет). Використайте «Новий Bash-термінал», щоб обійти лише для одного. Перевизначення для проєкту — у `.c11/agents.json`; для робочої області — в метаданих (`default_agent_use_bash`, `default_agent_inline`).",
        "모든 새 터미널 표면(메뉴, 키보드, 소켓)에 적용됩니다. 한 표면만 우회하려면 “새 Bash 터미널”을 사용하세요. 프로젝트별 재정의는 `.c11/agents.json`, 워크스페이스별 재정의는 워크스페이스 메타데이터(`default_agent_use_bash`, `default_agent_inline`)에 둡니다.",
        "应用于每个新终端表面（菜单、键盘、套接字）。使用“新建 Bash 终端”可对单个表面绕过。项目级覆盖位于 `.c11/agents.json`；工作区级覆盖位于工作区元数据 (`default_agent_use_bash`、`default_agent_inline`)。",
        "套用到每個新的終端機介面（選單、鍵盤、Socket）。使用「新增 Bash 終端機」可針對單一介面繞過。專案層級覆寫位於 `.c11/agents.json`；工作區層級覆寫位於工作區中繼資料 (`default_agent_use_bash`、`default_agent_inline`)。",
        "Применяется к каждому новому терминалу (меню, клавиатура, сокет). Используйте «Новый Bash-терминал», чтобы обойти для одного. Переопределения уровня проекта — в `.c11/agents.json`; уровня рабочей области — в метаданных (`default_agent_use_bash`, `default_agent_inline`).",
    ),
    (
